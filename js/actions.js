...
$(document).ready(function(){
	...
	$("body").delegate(".category","click",function(event){
		...
		var sort = $(".input-select-sort").find(":selected").text();
			...
			url		:	"action.php",
			method	:	"POST",
			data	:	{get_seleted_Category:1,cat_id:cid,get_amount:amount,get_sort:sort,pageNumber:1},
			success	:	function(data){
				$("#get_product").html(data);
				if($("body").width() < 480){
					$("body").scrollTop(683);
	...
	*/
	$("body").delegate(".selectBrand","click",function(event){
		...
		var sort = $(".input-select-sort").find(":selected").text();
			...
			url		:	"action.php",
			method	:	"POST",
			data	:	{selectBrand:1,brand_id:bid,get_amount:amount,get_sort:sort,pageNumber:1},
			success	:	function(data){
				$("#get_product").html(data);
				if($("body").width() < 480){
					$("body").scrollTop(683);
		...
		page("product_brand",bid);
	
	})
	$("body").delegate(".input-select-sort","change",function(event){
		event.preventDefault();
		if ($(".active").eq(0).hasClass("selectBrand")) {
			$("#get_product").html("<h3>Loading...</h3>");
			var bid = $(".btn.navbar-btn.selectBrand.active").attr('bid');
			var amount = $(".input-select-number").find(":selected").text();
			var sort = $(".input-select-sort").find(":selected").text();
		...
		else if ($(".active").eq(0).hasClass("category")) {
			$("#get_product").html("<h3>Loading...</h3>");
			var cid = $(".btn.navbar-btn.category.active").attr('cid');
			var amount = $(".input-select-number").find(":selected").text();
			var sort = $(".input-select-sort").find(":selected").text();
				$.ajax({
				url		:	"action.php",
	...
	*/
	...
	$("body").delegate(".allupdate","click",function(event){
		var allupdate = $(".update");
		allupdate.each(function() { 
			var update = $(this).parent().parent().parent();
			var update_id = update.find(".update").attr("update_id");
			var qty = update.find(".qty").val();
			$.ajax({
	...
	*/
	...
	page("product_cat","1");
	function page(col, value){
		$.ajax({
			url	:	"action.php",
			method	:	"POST",
			data	:	{page:1, col:col, value:value},
			success	:	function(data){
				$("#pageno").html(data);
			}
		})
	...
	$("body").delegate("#page","click",function(){
		...
		if ($(".active").eq(0).hasClass("selectBrand")) {
			...
			var sort = $(".input-select-sort").find(":selected").text();
				...
				url		:	"action.php",
				method	:	"POST",
				data	:	{selectBrand:1,brand_id:bid,get_amount:amount,get_sort:sort,pageNumber:pn},
				success	:	function(data){
					$("#get_product").html(data);
					if($("body").width() < 480){
						$("body").scrollTop(683);
			...
			var sort = $(".input-select-sort").find(":selected").text();
				...
				url		:	"action.php",
				method	:	"POST",
				data	:	{get_seleted_Category:1,cat_id:cid,get_amount:amount,get_sort:sort,pageNumber:pn},
				success	:	function(data){
					$("#get_product").html(data);
					if($("body").width() < 480){
						$("body").scrollTop(683);
	...
	$("#comment-form").on("click", function(event) { 
		$.ajax({
			url	:	"action.php",
			method	:	"GET",
			data	:	{comment:$("#comment-text").val(), p: $("#comment-product").val()},
			success	:	function(data){
				$("#get_comment").html(data);
			}
		})
	})
	function btn_profile(index) {
		$.ajax({
			url	:	"action.php",
			method	:	"POST",
			data	:	{getProfile:index},
			success	:	function(data){
				$("#get-content").html(data);
			}
		})
	}
	$("#profile").on("click",btn_profile(1));
	var $arr = $(".btn-profile");
    $arr.each(function() { 
		if ($(this).attr("index") == 2) {
			...
			$(this).on("click", function() {
				btn_profile($(this).attr("index"));
			})
		}
    })
	$("body").delegate("#edit-info","click", function() {
	...
	$("body").delegate("#change-pwd","click", function() {
